...
func (l *ApplicationLogic) CreateApplication(req *types.CreateApplicationRequest) (*model.Application, error) {
	// 检查应用编码是否存在
	var count int64
	l.db.Model(&model.Application{}).Where("code = ?", req.Code).Count(&count)
	if count > 0 {
...
func (l *ApplicationLogic) DeleteApplication(id uint) error {
	// 检查是否有角色使用该应用
	var roleCount int64
	l.db.Model(&model.Role{}).Where("app_id = ?", id).Count(&roleCount)
	if roleCount > 0 {
	...
	l.db.Model(&model.Resource{}).Where("app_id = ?", id).Count(&resourceCount)
	if resourceCount > 0 {
		return errors.New("该应用下有资源，无法删除")
	}

	return l.db.Delete(&model.Application{}, id).Error
}

// GetApplication 获取应用详情
func (l *ApplicationLogic) GetApplication(id uint) (*model.Application, error) {
	var app model.Application
	if err := l.db.First(&app, id).Error; err != nil {
		return nil, err
	}
	return &app, nil
}
...
func (l *ApplicationLogic) GetApplicationByCode(code string) (*model.Application, error) {
	var app model.Application
	if err := l.db.Where("code = ?", code).First(&app).Error; err != nil {
		return nil, err
	}
	return &app, nil
}
...
func (l *ApplicationLogic) ListApplications(req *types.ListApplicationsRequest) ([]model.Application, int64, error) {
	...
	query := l.db.Model(&model.Application{})
...
func (l *ApplicationLogic) GetAllApplications() ([]model.Application, error) {
	var apps []model.Application
	if err := l.db.Where("status = 1").Order("sort ASC").Find(&apps).Error; err != nil {
		return nil, err
	}
	return apps, nil
}
