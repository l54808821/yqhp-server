...
CREATE TABLE IF NOT EXISTS `t_mcp_server` (
    ...
    `description` VARCHAR(500) DEFAULT NULL COMMENT '描述',
    `transport` VARCHAR(20) NOT NULL COMMENT '传输方式: stdio/sse/streamable-http',
    `command` VARCHAR(500) DEFAULT NULL COMMENT 'stdio模式命令',
    `args` JSON DEFAULT NULL COMMENT 'stdio模式参数',
    `url` VARCHAR(500) DEFAULT NULL COMMENT 'sse/streamable-http模式URL',
    `headers` JSON DEFAULT NULL COMMENT '自定义HTTP请求头',
...
-- ============================================
-- 17. AI Skill 元数据表 (t_skill)
-- 元数据从 SKILL.md frontmatter 同步，用于列表展示、搜索、AI 执行
-- ============================================
CREATE TABLE IF NOT EXISTS `t_skill` (
    ...
    `is_delete` TINYINT(1) DEFAULT 0,
    `created_by` BIGINT UNSIGNED DEFAULT NULL COMMENT '创建人ID',
    -- 元数据（从 SKILL.md frontmatter 同步）
    `name` VARCHAR(100) NOT NULL COMMENT '显示名称',
    `slug` VARCHAR(64) DEFAULT NULL COMMENT '标准化名称（kebab-case）',
    `description` VARCHAR(1024) DEFAULT NULL COMMENT 'Skill 描述',
    `icon` VARCHAR(100) DEFAULT NULL COMMENT '图标标识',
    `category` VARCHAR(50) DEFAULT NULL COMMENT '分类: 编程/写作/分析/翻译等',
    `tags` JSON DEFAULT NULL COMMENT '标签',
    -- Agent Skills 规范字段（从 SKILL.md frontmatter 同步）
    `license` VARCHAR(500) DEFAULT NULL COMMENT '许可证',
    `compatibility` VARCHAR(500) DEFAULT NULL COMMENT '环境要求',
    `metadata_json` JSON DEFAULT NULL COMMENT '扩展元数据',
    `allowed_tools` TEXT DEFAULT NULL COMMENT '预批准工具列表',
    -- 来源信息
    `author` VARCHAR(100) DEFAULT '' COMMENT '作者/来源',
    `source_url` VARCHAR(500) DEFAULT '' COMMENT '来源 URL',
    `install_count` INT DEFAULT 0 COMMENT '安装量（展示用）',
    -- 管理字段
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='AI Skill 元数据表';

-- ============================================
-- 18. Skill 文件表 (t_skill_resource)
-- Skill = 一个文件夹，所有文件（含 SKILL.md）都存在这里
-- ============================================
CREATE TABLE IF NOT EXISTS `t_skill_resource` (
    ...
    `path` VARCHAR(500) NOT NULL COMMENT '相对路径（如 SKILL.md, scripts/base.py, validators/pptx.py）',
    `content` LONGTEXT DEFAULT NULL COMMENT '文件内容',
    ...
    UNIQUE KEY `uk_skill_resource_path` (`skill_id`, `path`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Skill 文件表';

-- ============================================
-- 内置 Skill 初始数据
-- 先插入 t_skill 元数据，再插入 t_skill_resource 存 SKILL.md 文件
-- ============================================
INSERT INTO `t_skill` (`created_at`, `updated_at`, `is_delete`, `name`, `slug`, `description`, `icon`, `category`, `tags`, `license`, `type`, `is_public`, `version`, `sort`, `status`) VALUES
(NOW(), NOW(), 0, '代码审查专家', 'code-review', '对代码进行专业的 Code Review，指出潜在问题、安全隐患和优化建议。Use when reviewing code quality, finding bugs, or checking best practices.', 'lucide:search-code', '编程', '["代码审查","Code Review","最佳实践"]', 'Apache-2.0', 1, 1, '1.0.0', 100, 1),
(NOW(), NOW(), 0, '测试用例生成器', 'test-case-generator', '根据需求描述或代码自动生成测试用例。Use when generating test cases, unit tests, or test plans from requirements or code.', 'lucide:test-tubes', '编程', '["测试","单元测试","用例生成"]', 'Apache-2.0', 1, 1, '1.0.0', 90, 1),
(NOW(), NOW(), 0, 'SQL 专家', 'sql-expert', '精通 SQL 编写、优化和问题排查。Use when writing SQL queries, optimizing slow queries, or designing database schemas.', 'lucide:database', '编程', '["SQL","数据库","性能优化"]', 'Apache-2.0', 1, 1, '1.0.0', 80, 1),
(NOW(), NOW(), 0, 'API 文档生成器', 'api-doc-generator', '根据代码自动生成清晰的 API 接口文档。Use when generating API documentation from code or interface definitions.', 'lucide:file-text', '编程', '["API","文档","接口"]', 'Apache-2.0', 1, 1, '1.0.0', 70, 1),
(NOW(), NOW(), 0, '翻译助手', 'translation', '专业的多语言翻译，保持原文语气和上下文准确性。Use when translating text between languages or localizing content.', 'lucide:languages', '翻译', '["翻译","多语言","本地化"]', 'Apache-2.0', 1, 1, '1.0.0', 60, 1),
(NOW(), NOW(), 0, '数据分析师', 'data-analysis', '分析数据并提供洞察、趋势和可视化建议。Use when analyzing datasets, finding patterns, or generating data-driven insights.', 'lucide:bar-chart-3', '分析', '["数据分析","统计","报表"]', 'Apache-2.0', 1, 1, '1.0.0', 50, 1),
(NOW(), NOW(), 0, '技术文档写手', 'tech-writing', '撰写清晰专业的技术文档、README和架构设计文档。Use when writing technical documentation, READMEs, or architecture design docs.', 'lucide:pen-line', '写作', '["技术文档","README","架构设计"]', 'Apache-2.0', 1, 1, '1.0.0', 40, 1),
(NOW(), NOW(), 0, '接口测试分析', 'api-test-analysis', '分析 HTTP 接口响应，判断接口行为是否符合预期。Use when analyzing HTTP responses, validating API behavior, or debugging API issues.', 'lucide:activity', '测试', '["接口测试","HTTP","断言"]', 'Apache-2.0', 1, 1, '1.0.0', 30, 1);

-- 为每个内置 Skill 插入 SKILL.md 文件到 t_skill_resource
INSERT INTO `t_skill_resource` (`skill_id`, `path`, `content`, `content_type`, `size`) VALUES
((SELECT id FROM t_skill WHERE slug='code-review'), 'SKILL.md', '---\nname: code-review\ndescription: "对代码进行专业的 Code Review，指出潜在问题、安全隐患和优化建议。Use when reviewing code quality, finding bugs, or checking best practices."\nlicense: Apache-2.0\n---\n\n# 代码审查专家\n\n你是一位资深的代码审查专家。请对用户提供的代码进行全面审查，包括：\n\n1. **代码质量**：命名规范、代码结构、可读性\n2. **潜在 Bug**：空指针、边界条件、并发问题\n3. **安全隐患**：注入攻击、敏感信息泄露、权限问题\n4. **性能优化**：算法复杂度、资源泄漏、不必要的开销\n5. **最佳实践**：设计模式、SOLID 原则、项目规范\n\n请用清晰的格式逐条列出问题，并给出改进建议和示例代码。严重程度标注为：🔴 严重 / 🟡 警告 / 🔵 建议。', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='test-case-generator'), 'SKILL.md', '---\nname: test-case-generator\ndescription: "根据需求描述或代码自动生成测试用例。Use when generating test cases, unit tests, or test plans from requirements or code."\nlicense: Apache-2.0\n---\n\n# 测试用例生成器\n\n你是一位测试工程专家。请根据用户提供的需求描述或代码，生成全面的测试用例。\n\n要求：\n1. 覆盖正常场景、边界条件和异常场景\n2. 每个用例包含：用例名称、前置条件、测试步骤、预期结果\n3. 如果提供了代码，额外生成单元测试代码\n4. 关注等价类划分和边界值分析\n5. 考虑安全性和性能相关测试场景', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='sql-expert'), 'SKILL.md', '---\nname: sql-expert\ndescription: "精通 SQL 编写、优化和问题排查。Use when writing SQL queries, optimizing slow queries, or designing database schemas."\nlicense: Apache-2.0\n---\n\n# SQL 专家\n\n你是一位数据库和 SQL 专家，精通 MySQL、PostgreSQL 等主流数据库。\n\n你的职责：\n1. 根据需求编写高效的 SQL 查询语句\n2. 分析和优化慢 SQL，给出执行计划解读\n3. 设计合理的表结构和索引策略\n4. 排查 SQL 相关问题（死锁、性能瓶颈等）', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='api-doc-generator'), 'SKILL.md', '---\nname: api-doc-generator\ndescription: "根据代码自动生成清晰的 API 接口文档。Use when generating API documentation from code or interface definitions."\nlicense: Apache-2.0\n---\n\n# API 文档生成器\n\n请根据用户提供的代码或接口信息，生成标准的 API 文档，包含：接口概述、请求参数、请求示例、响应格式、错误码、注意事项。', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='translation'), 'SKILL.md', '---\nname: translation\ndescription: "专业的多语言翻译，保持原文语气和上下文准确性。Use when translating text between languages or localizing content."\nlicense: Apache-2.0\n---\n\n# 翻译助手\n\n你是一位专业的多语言翻译专家。翻译原则：准确性、自然性、一致性、语境感知。', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='data-analysis'), 'SKILL.md', '---\nname: data-analysis\ndescription: "分析数据并提供洞察、趋势和可视化建议。Use when analyzing datasets, finding patterns, or generating data-driven insights."\nlicense: Apache-2.0\n---\n\n# 数据分析师\n\n请对用户提供的数据进行深入分析：数据概览、统计分析、深度洞察、可视化建议、行动建议。', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='tech-writing'), 'SKILL.md', '---\nname: tech-writing\ndescription: "撰写清晰专业的技术文档、README和架构设计文档。Use when writing technical documentation, READMEs, or architecture design docs."\nlicense: Apache-2.0\n---\n\n# 技术文档写手\n\n写作原则：结构清晰、简洁准确、读者友好、示例丰富、可维护性。', 'text/markdown', 0),
((SELECT id FROM t_skill WHERE slug='api-test-analysis'), 'SKILL.md', '---\nname: api-test-analysis\ndescription: "分析 HTTP 接口响应，判断接口行为是否符合预期。Use when analyzing HTTP responses, validating API behavior, or debugging API issues."\nlicense: Apache-2.0\n---\n\n# 接口测试分析\n\n请分析 HTTP 请求和响应：状态码、响应体、性能、一致性、安全。给出结论：✅ 通过 / ❌ 失败 / ⚠️ 警告。', 'text/markdown', 0);
